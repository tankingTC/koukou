...
public class KoukouApiService {
    ...
    public AuthResult login(String account, String password) throws IOException, ApiException {
        ...
        JsonObject json = executeJson(buildPost("/api/login", body, null));
    ...
    public AuthResult register(String nickname, String account, String password) throws IOException, ApiException {
        ...
        body.addProperty("account", safe(account));
        body.addProperty("password", safe(password));
        JsonObject json = executeJson(buildPost("/api/register", body, null));
        return parseAuthResult(json);
    }

    public String generateAvailableKoukouId() throws IOException, ApiException {
        JsonObject json = executeJson(buildGet("/api/users/available-id", null));
    ...
    public UserEntity findUser(String identifier, String authToken) throws IOException, ApiException {
        JsonObject json = executeJson(buildGet("/api/users/" + safe(identifier), authToken));
    ...
    public List<UserEntity> getFriends(String authToken) throws IOException, ApiException {
        JsonObject json = executeJson(buildGet("/api/friends", authToken));
    ...
    public List<FriendRequestEntity> getIncomingFriendRequests(String authToken) throws IOException, ApiException {
        JsonObject json = executeJson(buildGet("/api/friends/requests", authToken));
    ...
    public void sendFriendRequest(String authToken, String targetId, String message) throws IOException, ApiException {
        ...
        executeJson(buildPost("/api/friends/requests", body, authToken));
    }

    public void acceptFriendRequest(String authToken, String requestId) throws IOException, ApiException {
        executeJson(buildPost("/api/friends/requests/" + safe(requestId) + "/accept", new JsonObject(), authToken));
    }

    public void rejectFriendRequest(String authToken, String requestId) throws IOException, ApiException {
        executeJson(buildPost("/api/friends/requests/" + safe(requestId) + "/reject", new JsonObject(), authToken));
    }

    public void deleteFriend(String authToken, String friendId) throws IOException, ApiException {
        executeJson(buildDelete("/api/friends/" + safe(friendId), authToken));
    }

    private Request buildGet(String path, String authToken) {
        Request.Builder builder = new Request.Builder().url(buildUrl(path)).get();
        addBearer(builder, authToken);
        return builder.build();
    }

    private Request buildDelete(String path, String authToken) {
        Request.Builder builder = new Request.Builder().url(buildUrl(path)).delete();
        addBearer(builder, authToken);
        return builder.build();
    }

    private Request buildPost(String path, JsonObject body, String authToken) {
    ...
    private JsonObject executeJson(Request request) throws IOException, ApiException {
        try (Response response = client.newCall(request).execute()) {
            String body = response.body() == null ? "" : response.body().string();
            JsonObject json = parseObject(body);
            if (!response.isSuccessful()) {
                throw toApiException(response.code(), json, body);
            }
            return json;
        }
    }
